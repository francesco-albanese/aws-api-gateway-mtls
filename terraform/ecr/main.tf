...
resource "aws_ecr_repository" "token_lambda" {
  name                 = "${var.project_name}-token-lambda"
  image_tag_mutability = "MUTABLE"
  force_delete         = false
...
resource "aws_ecr_lifecycle_policy" "token_lambda" {
  repository = aws_ecr_repository.token_lambda.name
